...
def im_list_to_blob(ims):
    ...
    if not isinstance(ims, list):
        ims = [ims]
    max_shape = np.array([im.shape for im in ims]).max(axis=0)    # 得到所有数据集中长宽最大值
    ...
    channel_swap = (0, 3, 1, 2)    #设置矩阵维度变换的参数，最终的4个维度分别对应图片、通道、高、宽，这与caffe的blob的格式就保持一致了
...
def prep_im_for_blob(im, pixel_means, target_size, max_size):
    ...
    im_scale = float(target_size) / float(im_size_min)              # 尺度比例
    # Prevent the biggest axis from being more than max_size 判断会不会超出最大尺度来调整比例,至少有一条是600的尺度
