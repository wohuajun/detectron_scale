...
def train_model():
    ...
    #setup_model_for_training(model, weights_file, output_dir)   #　　打印XＸ/64的就是这一部分
    setup_model_for_training(model, weights_file, output_dir)
    # print("----ａａａａａａａａａａａａａａａａａａ--",a)
    # print("train  60---------------------------------------------------------------")
    training_stats = TrainingStats(model)
    CHECKPOINT_PERIOD = int(cfg.TRAIN.SNAPSHOT_ITERS / cfg.NUM_GPUS)
    # print("train 62 -----------------",CHECKPOINT_PERIOD)
...
def add_model_training_inputs(model):
    ...
    # print("Trian.py文件  197,roidb的长度为{}".format(len(roidb)))     # 如果flip设置为true的话，则比原始数据数量多了
