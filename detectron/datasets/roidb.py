...
def combined_roidb_for_training(dataset_names, proposal_files):
    ...
    def get_roidb(dataset_name, proposal_file):
        ...
        #print("get_roidb函数,roidb的数量为{}".format(len(roidb)))     # 这里的数据数还是原始的数量
    ...
    print("roidb.py 74 在train文件中调用了我(combined_roidb_for_training)来返回roidb,共计长度为{}".format(len(roidb)))   # 已经数据多了
...
def filter_for_training(roidb):
    ...
    if NEGATIVE:
        filtered_roidb = roidb #[entry for entry in roidb if is_valid(entry)]  保留所有的roidb的数据
        print("---------负样本有效-----------")
        #print(filtered_roidb)
        #print("--------------------")
    else:
        filtered_roidb = [entry for entry in roidb if is_valid(entry)]  #保留所有的roidb的数据
