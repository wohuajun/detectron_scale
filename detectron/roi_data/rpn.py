...
def _get_rpn_blobs(im_height, im_width, foas, all_anchors, gt_boxes):
    ...
    if straddle_thresh >= 0:
        ...
        inds_inside = np.where(
         ...
         #   因为返回的为array((),)所以取[0]的数据
        # print("~~~~~~~~~~~~~~~~~~~~~~~~",inds_inside)   #  符合条件的 index    
        # keep only inside anchors 
        anchors = all_anchors[inds_inside, :]
        # print("@@@@@@@@@@@@@@@@@@@@@@@@{}@@@@@@@@@@@@@@@@{}".format(len(anchors),len(inds_inside)))
    else:
...
# ----------------------------------------------------------------------------------------------------
    if len(gt_boxes) == 0:
        enable_inds =  npr.randint(num_inside, size=256)     # 在总共符合条件的非负框中,随机256个框
        labels[enable_inds] = 0                              # 将对应的label设置为0
        bg_inds = np.where(labels == 0)[0]                   # 
        # print("@@@@%%%%##$#{}{}".format(enable_inds, bg_inds))        
        ...
        #print("~~~~~~~~~~~~~~0000000~~~~~~~~~~~~~~~~~~~~",np.where(bbox_outside_weights == 1.0 / num_examples)[0])  
        #bg_inds = np.where(labels == 0)[0]
        #print('aa')
# -----------------------------------------------------------------------------------------------------
    if len(gt_boxes) > 0:
