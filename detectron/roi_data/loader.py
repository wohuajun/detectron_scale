...
from detectron.roi_data.minibatch import get_minibatch_mul                    # -----------
...
class RoIDataLoader(object):
    ...
    def get_next_minibatch(self):
        ...
        while not valid:
            ...
            # db_inds 返回的是一个数字，应该就是数据的 index, db_inds的长度和__C.TRAIN.IMS_PER_BATCH 有关　　
            #print("loader.py 138 ----------{}-------{}".format(len(db_inds),db_inds))
            minibatch_db = [self._roidb[i] for i in db_inds]
            #print("loader.py 141 ----------roidb的长度{}".format(len(self._roidb)))
            # --------------------------------------是否采取多尺度扩充数据----------------------------------------
            # roidb_noclass = dict()
            if MULTI_SCALE:
            #    blobs, valid = get_minibatch_s6(minibatch_db,roidb_noclass)
                blobs, valid = get_minibatch_mul(minibatch_db)
            # ------------------------------------------------------------------------------------------------
            else:
                # print("执行无扩增版本~~~~~~~~~~~~~~~~~~~~~~~")
    ...
    def _get_next_minibatch_inds(self):
        ...
        with self._lock:
            ...
            if self._cur >= len(self._perm):
                self._shuffle_roidb_inds()
            # print("loader.py 189    db_inds{}".format(db_inds))
        return db_inds
...
class RoIDataLoader_train(object):
    def __init__(
        self,
        roidb,
        num_loaders=4,
